...
2. I wrote unit test for every existing rules before refactoring, to be able to check the result.
3. I refactored `updateBenefitValue`. Each drug has its own update function. Drug exposes small methods and limits are stored in const.
4. I added Dafalgan with TDD.

I have some ideas for improvement : 
- create a unit of increase / decrease : Here the code can increase or decrease with the basic unit of 1, but with a const we could fix and change if necessary this unit in order to be more flexible. For ample if unit became 2 instead of one, doliprane would decrease of 2 before expiration and of 4 after expiration, and twice for Dafalgan

- At the end of the process we could create function for each drug which return the number to add to benefit (positive or negative). So for the dafalgan we could use the basic drugs function and just add twice the return value instead of implement a new function specific for dafalgan
